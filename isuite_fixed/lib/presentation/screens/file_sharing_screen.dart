...
class _FileSharingScreenState extends State<FileSharingScreen> {
  ...
  Widget _buildActiveTransfersSection(FileSharingProvider provider) => Card(
          ...
          child: Column(
            ...
            children: [
              ...
              ...provider.activeTransfers
                  .take(3)
                  .map((transfer) => _buildTransferItem(transfer, provider)),
  ...
  Widget _buildTransferItem(
          FileTransferModel transfer, FileSharingProvider provider) =>
      Padding(
  ...
  Widget _buildConnectionCard(BuildContext context, FileSharingModel connection,
      FileSharingProvider provider) {
    ...
    return Semantics(
      ...
      hint:
          'Status: ${isActive ? 'Active' : 'Inactive'}. Protocol: ${connection.protocolText}. Tap to manage.',
      child: Card(
                ...
                Row(
                  children: [
                    ...
                    PopupMenuButton<String>(
                      onSelected: (action) =>
                          _handleConnectionAction(action, connection, provider),
                      itemBuilder: (context) => [
                        PopupMenuItem(
                          value: 'connect',
                          child: Row(
  ...
  void _handleConnectionAction(String action, FileSharingModel connection,
      FileSharingProvider provider) {
    switch (action) {
      case 'connect':
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<FileSharingProtocol>(
                  ...
                  items: FileSharingProtocol.values
                ...
                TextFormField(
                  ...
                  decoration:
                      const InputDecoration(labelText: 'Local Path (optional)'),
                ),
              ],
            ),
          ),
              ...
              if (_formKey.currentState?.validate() ?? false) {
                final connection = FileSharingModel(
                  ...
                  password: _passwordController.text.isNotEmpty
                      ...
                      : null,
                  remotePath: _remotePathController.text.isNotEmpty
                      ? _remotePathController.text
                      : null,
                  localPath: _localPathController.text.isNotEmpty
  ...
  void _showConnectionDetails(BuildContext context, FileSharingModel connection,
      FileSharingProvider provider) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
          ...
          child: Column(
            ...
            children: [
              ...
              if (connection.lastConnected != null)
                _buildDetailRow(
                    'Last Connected', connection.lastConnected!.toString()),
            ],
          ),
        ),
        actions: [
  ...
  Future<void> _pickFileForUpload(
      FileSharingModel connection, FileSharingProvider provider) async {
  ...
  void _showDownloadDialog(
      FileSharingModel connection, FileSharingProvider provider) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              if (remoteFileController.text.isNotEmpty &&
                  localPathController.text.isNotEmpty) {
                await provider.downloadFile(connection,
                    remoteFileController.text, localPathController.text);
                Navigator.of(context).pop();
              }
            },
  ...
  void _showDeleteConnectionDialog(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content:
            Text('Delete ${connection.name}? This action cannot be undone.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('Cancel'),
