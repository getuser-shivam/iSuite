...
class AdvancedSettingsSystem {
  static final AdvancedSettingsSystem _instance = AdvancedSettingsSystem._internal();
  ...
  Future<bool> setSetting(String key, dynamic value) async {
    ...
    if (!validationResult.isValid) {
      _eventController.add(SettingsEvent.validationFailed(key, validationResult.errors));
      return false;
    }
  ...
  Future<ImportResult> importSettings(String jsonData) async {
    try {
      ...
      _eventController.add(SettingsEvent.settingsImported(imported, skipped, errors));
  ...
  Map<String, dynamic> getStatistics() {
    ...
    for (final setting in _settings.values) {
      categories[setting.categoryId] = (categories[setting.categoryId] ?? 0) + 1;
    }
  ...
  void _setupEventListeners() {
    ...
    _centralConfig.onConfigChanged.listen((event) {
      ...
      if (_settings.containsKey(key) && _currentValues[key] != newValue) {
        ...
        _eventController.add(SettingsEvent.settingChanged(key, newValue, event.oldValue));
      }
    });
  }
...
class SettingsSearch {
  List<Setting> search(String query, List<Setting> settings, {List<String>? categories}) {
    ...
    return settings.where((setting) {
      // Category filter
      if (categories != null && categories.isNotEmpty && !categories.contains(setting.categoryId)) {
        return false;
      }

      // Text search
      if (query.isNotEmpty) {
        final searchText = '${setting.name} ${setting.description} ${setting.key}'.toLowerCase();
...
class SettingsBackup {
  Future<String> createBackup(Map<String, dynamic> values, Map<String, Setting> settings) async {
    final backupData = {
      'version': '1.0',
  ...
  Future<RestoreResult> restoreBackup(String backupData, AdvancedSettingsSystem settingsSystem) async {
    try {
...
abstract class SettingsEvent {
  ...
  factory SettingsEvent.settingRegistered(Setting setting) = SettingRegisteredEvent;

  factory SettingsEvent.categoryRegistered(SettingsCategory category) = CategoryRegisteredEvent;

  factory SettingsEvent.settingChanged(String key, dynamic newValue, dynamic oldValue) =
      SettingChangedEvent;

  factory SettingsEvent.validationFailed(String key, List<String> errors) = ValidationFailedEvent;

  factory SettingsEvent.allSettingsReset() = AllSettingsResetEvent;

  factory SettingsEvent.profileCreated(SettingsProfile profile) = ProfileCreatedEvent;

  factory SettingsEvent.profileLoaded(SettingsProfile profile) = ProfileLoadedEvent;

  factory SettingsEvent.profileDeleted(String profileId) = ProfileDeletedEvent;

  factory SettingsEvent.settingsImported(int imported, int skipped, List<String> errors) =
...
class SettingRegisteredEvent extends SettingsEvent {
  ...
  SettingRegisteredEvent(this.setting) : super('setting_registered', DateTime.now());
}

class CategoryRegisteredEvent extends SettingsEvent {
  final SettingsCategory category;

  CategoryRegisteredEvent(this.category) : super('category_registered', DateTime.now());
}
...
class SettingChangedEvent extends SettingsEvent {
  ...
  SettingChangedEvent(this.key, this.newValue, this.oldValue) : super('setting_changed', DateTime.now());
}
...
class ValidationFailedEvent extends SettingsEvent {
  ...
  ValidationFailedEvent(this.key, this.errors) : super('validation_failed', DateTime.now());
}
...
class ProfileDeletedEvent extends SettingsEvent {
  ...
  ProfileDeletedEvent(this.profileId) : super('profile_deleted', DateTime.now());
}
...
class SettingsImportedEvent extends SettingsEvent {
  ...
  SettingsImportedEvent(this.imported, this.skipped, this.errors) : super('settings_imported', DateTime.now());
}
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ExpansionTile(
          ...
          children: [
            ..._settings.getSettingsInCategory(category.id)
...
class _SettingDialogState extends State<SettingDialog> {
  ...
  void initState() {
    super.initState();
    _value = _settings.getSetting(widget.setting.key) ?? widget.setting.defaultValue;

    if (widget.setting.type == SettingType.string || widget.setting.type == SettingType.file) {
      ...
      case SettingType.number:
        return TextField(
          ...
          decoration: InputDecoration(
            labelText: 'Value',
            hintText: widget.setting.minValue != null && widget.setting.maxValue != null
            ...
            if (profiles.isEmpty)
              ...
              ...profiles.map((profile) => ListTile(
                ...
                trailing: PopupMenuButton<String>(
                  onSelected: (action) => _handleProfileAction(profile.id, action),
                  itemBuilder: (context) => [
