...
class CentralConfig {
  ...
  /// Register a component with the configuration system
  Future<void> registerComponent(
    ...
    await _lock.write(() async {
      ...
      _componentOverrides[componentName] = defaultParameters ?? {};

      // Register relationships
      if (dependencies != null) {
        for (final dep in dependencies) {
          _componentRelationships.add(ComponentRelationship(
      ...
      if (provides != null) {
        for (final service in provides) {
          _componentRelationships.add(ComponentRelationship(
            sourceComponent: componentName,
            targetComponent: service,
            type: RelationshipType.provides_to,
          ));
        }
      }

      _notifyEvent(ConfigEvent(
  ...
  List<String> getInitializationOrder() {
    ...
    void visit(String component) {
      ...
      if (visiting.contains(component)) {
        throw Exception('Circular dependency detected: $component');
      }

      visiting.add(component);

      // Visit dependencies first
      final dependencies = _componentRelationships
          .where((rel) =>
              rel.sourceComponent == component &&
              rel.type == RelationshipType.depends_on)
          .map((rel) => rel.targetComponent);
    ...
    for (final component in _componentOverrides.keys) {
