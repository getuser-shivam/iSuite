...
class SecureApiClient {
  ...
  Future<ApiResponse<T>> authenticatedRequest<T>(
    ...
    try {
      ...
      if (!_checkRateLimit(endpoint)) {
        throw ApiException.rateLimitExceeded('Rate limit exceeded for endpoint: $endpoint');
      }
      ...
      if (enableEncryption && _enableEncryption) {
        processedRequest = await _encryptionManager.encryptRequest(processedRequest);
      }

      // Sign request if enabled
      if (enableSigning && _enableSigning) {
        processedRequest = await _signatureManager.signRequest(processedRequest);
      }
      ...
      // Record metrics
      _recordMetrics(endpoint, method, DateTime.now().difference(startTime), true);
      ...
      _recordMetrics(endpoint, method, DateTime.now().difference(startTime), false);
      ...
      throw ApiException.networkError('Request failed: $e', stackTrace: stackTrace);
    }
  }
  ...
  Future<ApiResponse<String>> uploadFile(
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        _recordMetrics(endpoint, 'POST', DateTime.now().difference(startTime), true);
      ...
      _recordMetrics(endpoint, 'POST', DateTime.now().difference(startTime), false);

      if (e is ApiException) {
        rethrow;
      }

      throw ApiException.networkError('Upload failed: $e', stackTrace: stackTrace);
    }
  }
  ...
  Future<ApiResponse<File>> downloadFile(
    ...
    try {
      ...
      if (streamedResponse.statusCode >= 200 && streamedResponse.statusCode < 300) {
        ...
        _recordMetrics(endpoint, 'GET', DateTime.now().difference(startTime), true);
      ...
      _recordMetrics(endpoint, 'GET', DateTime.now().difference(startTime), false);

      if (e is ApiException) {
        rethrow;
      }

      throw ApiException.networkError('Download failed: $e', stackTrace: stackTrace);
    }
  }
  ...
  /// Record API metrics
  void _recordMetrics(String endpoint, String method, Duration duration, bool success) {
  ...
  void _collectMetrics() {
    final summary = ApiMetricsSummary(
      ...
      successfulRequests: _metrics.values.fold(0, (sum, m) => sum + m.successfulRequests),
      failedRequests: _metrics.values.fold(0, (sum, m) => sum + m.failedRequests),
      averageResponseTime: _metrics.values.isNotEmpty
          ? _metrics.values.map((m) => m.averageResponseTime).reduce((a, b) => a + b) / _metrics.values.length
          : Duration.zero,
      endpoints: _metrics,
    );
...
class SignatureManager {
  static const String _secret = 'your-hmac-secret-key-here'; // Should be from secure storage

  Future<ApiRequest> signRequest(ApiRequest request) async {
    final payload = '${request.method}${request.endpoint}${request.timestamp.millisecondsSinceEpoch}';
    final signature = _generateSignature(payload);

    return request.copyWith(
      headers: {...request.headers, 'X-Signature': signature, 'X-Timestamp': request.timestamp.millisecondsSinceEpoch.toString()},
    );
  }
  ...
  Future<void> verifyResponse(ApiResponse response) async {
    ...
    if (timestamp == null) throw ApiException.securityError('Missing timestamp in response');

    final expectedSignature = _generateSignature('response_$timestamp');
    if (signature != expectedSignature) {
      throw ApiException.securityError('Response signature verification failed');
    }
  }
...
class ApiException implements Exception {
  ...
  factory ApiException.networkError(String message, {StackTrace? stackTrace}) {
    return ApiException(ApiExceptionType.networkError, message, stackTrace: stackTrace);
  }

  factory ApiException.serverError(String message, {int? statusCode, StackTrace? stackTrace}) {
    return ApiException(ApiExceptionType.serverError, message, statusCode: statusCode, stackTrace: stackTrace);
  }
...
class ApiMetrics {
  ...
  Duration get averageResponseTime {
    return totalRequests > 0 ? totalResponseTime ~/ totalRequests : Duration.zero;
  }
...
abstract class ApiEvent {
  ...
  factory ApiEvent.metricsCollected(ApiMetricsSummary summary) = MetricsCollectedEvent;
}
...
class MetricsCollectedEvent extends ApiEvent {
  ...
  MetricsCollectedEvent(this.summary) : super('metrics_collected', DateTime.now());
}
