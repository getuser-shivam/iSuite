...
class InternationalizationSystem {
  static final InternationalizationSystem _instance = InternationalizationSystem._internal();
  ...
  Future<void> _loadFallbackTranslations() async {
    try {
      _fallbackTranslations = await _translationLoader.loadTranslations(_fallbackLanguage);
    } catch (e) {
    ...
    try {
      ...
      if (translations == null) {
        ...
        if (_enableValidation) {
          final validationResult = await _validationEngine.validateTranslations(translations, _fallbackTranslations);
          if (!validationResult.isValid) {
            debugPrint('Translation validation warnings: ${validationResult.warnings}');
          }
        }

        // Cache the translations
        await _translationCache.storeTranslations(languageCode, translations, _cacheExpiration);
      }

      _translations[languageCode] = translations;
      _translationController.add(TranslationEvent.translationsLoaded(languageCode, translations.length));
      ...
      _translationController.add(TranslationEvent.translationLoadFailed(languageCode, e.toString()));
    }
  }
  ...
  Future<void> changeLocale(Locale locale) async {
    ...
    _translationController.add(TranslationEvent.localeChanged(oldLocale, locale));
  ...
  String translate(
    ...
    if (translation == null) {
      ...
      if (languageCode != _fallbackLanguage) {
        final fallbackTranslations = _translations[_fallbackLanguage] ?? _fallbackTranslations;
    ...
    // Apply pluralization
    if (count != null) {
      translation = _pluralizationEngine.applyPluralization(translation, count, languageCode);
    }
  ...
  String? _findTranslation(String key, Map<String, dynamic> translations, String? context) {
    // Direct key lookup
  ...
  String formatDate(DateTime date, {String? format, Locale? locale}) {
    final targetLocale = locale ?? _currentLocale;
    return _formattingEngine.formatDate(date, format: format, locale: targetLocale);
  }

  /// Format time according to locale
  String formatTime(DateTime time, {String? format, Locale? locale}) {
    final targetLocale = locale ?? _currentLocale;
    return _formattingEngine.formatTime(time, format: format, locale: targetLocale);
  }

  /// Format number according to locale
  String formatNumber(num number, {String? format, Locale? locale}) {
    final targetLocale = locale ?? _currentLocale;
    return _formattingEngine.formatNumber(number, format: format, locale: targetLocale);
  }

  /// Format currency according to locale
  String formatCurrency(num amount, String currencyCode, {Locale? locale}) {
    final targetLocale = locale ?? _currentLocale;
    return _formattingEngine.formatCurrency(amount, currencyCode, locale: targetLocale);
  }
  ...
  double _calculateCoverage(Map<String, dynamic> translations, Map<String, dynamic> reference) {
  ...
  Stream<TranslationEvent> get translationEvents => _translationController.stream;
  ...
  Future<Map<String, dynamic>> loadTranslations(String languageCode) async {
    try {
      ...
      if (translations is! Map) {
        throw FormatException('Invalid translation file structure for $languageCode');
      }
...
class AnalyticsTracker {
  ...
  Future<void> trackMissingTranslation(String key, String languageCode, String? context) async {
    _analytics['missing_translations'] = (_analytics['missing_translations'] ?? 0) + 1;
  }
...
class ISuiteLocalizationsDelegate extends LocalizationsDelegate<ISuiteLocalizations> {
  const ISuiteLocalizationsDelegate();

  @override
  bool isSupported(Locale locale) {
    return InternationalizationSystem()._supportedLanguages.contains(locale.languageCode);
  }
...
abstract class TranslationEvent {
  ...
  factory TranslationEvent.initialized(Locale locale) = TranslationInitializedEvent;

  factory TranslationEvent.localeChanged(Locale oldLocale, Locale newLocale) =
      LocaleChangedEvent;

  factory TranslationEvent.translationsLoaded(String languageCode, int keyCount) =
      TranslationsLoadedEvent;

  factory TranslationEvent.translationLoadFailed(String languageCode, String error) =
      TranslationLoadFailedEvent;
}

class TranslationInitializedEvent extends TranslationEvent {
  final Locale locale;

  TranslationInitializedEvent(this.locale) : super('initialized', DateTime.now());
}
...
class LocaleChangedEvent extends TranslationEvent {
  ...
  LocaleChangedEvent(this.oldLocale, this.newLocale) : super('locale_changed', DateTime.now());
}
