...
class RetryService {
  ...
  final StreamController<RetryEvent> _eventController =
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      await _centralConfig.registerComponent('RetryService', '1.0.0',
      ...
      _loggingService.info(
          'Retry Service initialized successfully', 'RetryService');
    } catch (e, stackTrace) {
      _loggingService.error(
          'Failed to initialize Retry Service', 'RetryService',
  ...
  int get maxAttempts =>
      _centralConfig.getParameter(_defaultMaxAttempts, defaultValue: 3);
  int get baseDelayMs =>
      _centralConfig.getParameter(_defaultBaseDelay, defaultValue: 1000);
  int get maxDelayMs =>
      _centralConfig.getParameter(_defaultMaxDelay, defaultValue: 30000);
  double get backoffMultiplier =>
      _centralConfig.getParameter(_defaultBackoffMultiplier, defaultValue: 2.0);
  bool get jitterEnabled =>
      _centralConfig.getParameter(_defaultJitterEnabled, defaultValue: true);
  double get jitterPercent =>
      _centralConfig.getParameter(_defaultJitterPercent, defaultValue: 0.1);

  bool get networkRetryEnabled => _centralConfig
      .getParameter('retry.network_retry_enabled', defaultValue: true);
  bool get timeoutRetryEnabled => _centralConfig
      .getParameter('retry.timeout_retry_enabled', defaultValue: true);
  bool get serverErrorRetryEnabled => _centralConfig
      .getParameter('retry.server_error_retry_enabled', defaultValue: true);
  bool get authErrorRetryEnabled => _centralConfig
      .getParameter('retry.auth_error_retry_enabled', defaultValue: false);
  bool get clientErrorRetryEnabled => _centralConfig
      .getParameter('retry.client_error_retry_enabled', defaultValue: false);
  ...
  Future<T> execute<T>({
    ...
    final effectivePolicy =
        policy ?? _policies['default'] ?? _createDefaultPolicy();
    ...
    for (int attempt = 0; attempt <= effectivePolicy.maxAttempts; attempt++) {
      try {
        ...
        final result = await operation()
            .timeout(effectivePolicy.timeout ?? const Duration(seconds: 30));
  ...
  void resetStatistics(String operationName) {
    ...
    _loggingService.info(
        'Reset statistics for operation: $operationName', 'RetryService');
  }
  ...
  void _createDefaultPolicies() {
    ...
    _loggingService.info(
        'Created ${policies.length} default retry policies', 'RetryService');
  }
  ...
  RetryStatistics _getOrCreateStatistics(String operationName) {
    return _statistics.putIfAbsent(
        operationName, () => RetryStatistics(operationName));
  }

  /// Calculate delay for retry attempt
  Duration _calculateDelay(RetryPolicy policy, int attempt) {
    // Exponential backoff: baseDelay * (multiplier ^ attempt)
    double delay = policy.baseDelay.inMilliseconds *
        pow(policy.backoffMultiplier, attempt).toDouble();
    ...
    if (policy.jitterEnabled) {
      ...
      final jitter =
          (Random().nextDouble() - 0.5) * 2 * jitterRange; // ±jitterRange
      delay += jitter;
...
class RetryStatistics {
  ...
  double get failureRate =>
      totalAttempts > 0 ? failedAttempts / totalAttempts : 0.0;
  Duration get averageDuration =>
      totalAttempts > 0 ? totalDuration ~/ totalAttempts : Duration.zero;
