...
class FreeAIService {
  ...
  static const Map<AIModel, Map<String, dynamic>> _freeModels = {
    AIModel.gemma: {
      ...
      'context_length': 2048,
      'free': true,
      'endpoint': 'https://generativelanguage.googleapis.com/v1beta/models/gemma-2b-it:generateContent',
    },
    AIModel.llama: {
      'name': 'Llama 3.1 8B',
      'provider': 'Meta',
  ...
  final StreamController<AIEvent> _aiEventController = StreamController.broadcast();
  ...
  Future<void> initialize({AIModel? preferredModel}) async {
    ...
    try {
      ...
      await _config.registerComponent(
        ...
        parameters: {
          // Model selection
          'ai.model.active': (preferredModel ?? AIModel.gemma).toString().split('.').last,
      ...
      final configModel = await _config.getParameter<String>('ai.model.active', defaultValue: 'gemma');
      ...
      _logger.info('Free AI Service initialized with ${_activeModel.name} model', 'FreeAIService');
  ...
  Future<String> generateText(String prompt, {
    ...
    // Check cache first
    final cacheKey = _generateCacheKey(prompt, maxTokens, temperature, systemPrompt);
    ...
    try {
      _emitAIEvent(AIEventType.generationStarted, data: {'prompt_length': prompt.length});

      String response;

      // Try local Ollama first if enabled
      if (_activeModel == AIModel.ollama) {
        response = await _generateWithOllama(prompt, maxTokens, temperature, systemPrompt);
      } else {
        // Use free online APIs
        response = await _generateWithFreeAPI(prompt, maxTokens, temperature, systemPrompt);
      }

      // Cache the response
      _cacheResponse(cacheKey, response);

      _emitAIEvent(AIEventType.generationCompleted, data: {'response_length': response.length});
      ...
      final fallbackEnabled = await _config.getParameter<bool>('ai.model.fallback_enabled', defaultValue: true);
      if (fallbackEnabled && _activeModel != AIModel.gemma) {
        ...
        return await generateText(prompt, maxTokens: maxTokens, temperature: temperature, systemPrompt: systemPrompt);
      }
  ...
  Future<FileAnalysisResult> analyzeFile(String filePath, {
    ...
    // Check file size
    final maxSize = await _config.getParameter<int>('ai.file_analysis.max_size_mb', defaultValue: 10) * 1024 * 1024;
    final fileSize = await file.length();
    if (fileSize > maxSize) {
      throw Exception('File too large: ${fileSize ~/ (1024 * 1024)}MB (max: ${maxSize ~/ (1024 * 1024)}MB)');
    }

    try {
      _emitAIEvent(AIEventType.fileAnalysisStarted, data: {'file_path': filePath, 'file_size': fileSize});
      ...
      _emitAIEvent(AIEventType.fileAnalysisCompleted, data: {'file_path': filePath});
      _logger.info('File analysis completed: $filePath', 'FreeAIService');

      return result;

    } catch (e) {
      _logger.error('File analysis failed: $filePath', 'FreeAIService', error: e);
      rethrow;
    }
  }
  ...
  Future<List<String>> getSuggestions(String content, {
    ...
    try {
      ...
      _logger.debug('Generated ${suggestions.length} AI suggestions', 'FreeAIService');
      return suggestions;

    } catch (e) {
      _logger.error('AI suggestions generation failed', 'FreeAIService', error: e);
      return [];
    }
  }
  ...
  Future<void> switchModel(AIModel newModel) async {
    ...
    _logger.info('Switching AI model from ${_activeModel.name} to ${newModel.name}', 'FreeAIService');

    _activeModel = newModel;
    await _config.setParameter('ai.model.active', newModel.toString().split('.').last);

    // Clear cache when switching models
    _responseCache.clear();

    _emitAIEvent(AIEventType.modelSwitched, data: {'new_model': newModel.name});
    _logger.info('Successfully switched to ${newModel.name} model', 'FreeAIService');
  }
  ...
  Future<String> _generateWithOllama(String prompt, int maxTokens, double temperature, String? systemPrompt) async {
    final endpoint = await _config.getParameter<String>('ai.ollama.endpoint', defaultValue: 'http://localhost:11434');
  ...
  Future<String> _generateWithFreeAPI(String prompt, int maxTokens, double temperature, String? systemPrompt) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Free AI API error: ${response.statusCode} - ${response.body}');
    }
  }

  String _generateFileAnalysisPrompt(String filePath, String content, AnalysisType type) {
  ...
  String _generateSuggestionPrompt(String content, SuggestionType type, int maxSuggestions) {
    switch (type) {
  ...
  String _generateCacheKey(String prompt, int maxTokens, double temperature, String? systemPrompt) {
  ...
  void _cacheResponse(String key, String response) {
    final cacheEnabled = _config.getParameter('ai.cache.enabled', defaultValue: true);
    if (cacheEnabled) {
  ...
  bool _canMakeRequest() {
    final rateLimitingEnabled = _config.getParameter('ai.rate_limiting.enabled', defaultValue: true);
