...
class FileOperationsService {
  static Future<String> compressFiles(
  ...
  static Future<Map<String, dynamic>> calculateFileHashes(
  ...
  static Future<bool> verifyFileIntegrity(
      String filePath, Map<String, String> expectedHashes) async {
    try {
  ...
  static Future<String> createBackup(
      List<String> filePaths, String backupPath) async {
    try {
  ...
  static Future<List<String>> restoreFromBackup(
      String backupPath, String restorePath) async {
    try {
      ...
      for (final filePath in files) {
        ...
        if (await backupFile.exists()) {
          ...
          final isValid =
              await verifyFileIntegrity(backupFilePath, expectedHashes);
  ...
  static Future<Map<String, dynamic>> getDetailedFileInfo(
      String filePath) async {
    try {
      final file = File(filePath);
  ...
  static Future<void> setFilePermissions(
      String filePath, Map<String, bool> permissions) async {
    try {
      final file = File(filePath);
      final stat = await file.stat();
      var mode = stat.mode;
        ...
        if (file is File) {
          ...
          if (matches) {
            results.add({
              ...
              'matches':
                  _findContentMatches(content, searchTerm, caseSensitive),
            });
          }
        }
      }
  ...
  static List<String> _getContext(
      List<String> lines, int currentLine, int contextLines) {
  ...
  static Future<Map<String, dynamic>> analyzeStorageUsage(
  ...
  static Future<List<Map<String, dynamic>>> _findLargestFiles(
      String directoryPath,
      {int limit = 10}) async {
    try {
      final directory = Directory(directoryPath);
  ...
  static Future<List<Map<String, dynamic>>> _findDuplicateFiles(
      String directoryPath) async {
    try {
      final directory = Directory(directoryPath);
      final files = await directory.list(recursive: true).toList();
