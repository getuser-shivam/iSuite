...
class SupabaseManager {
  ...
  SupabaseConnectionState _connectionState = SupabaseConnectionState.disconnected;
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _logger.info('Supabase Manager initialized successfully with all services', 'SupabaseManager');
  ...
  Future<void> _initializeSupabaseClient() async {
    ...
    final supabaseAnonKey = await _config.getParameter<String>('supabase.anon_key');
  ...
  Future<Map<String, String>> _buildEnhancedHeaders() async {
    final anonKey = await _config.getParameter<String>('supabase.anon_key') ?? '';
    final clientVersion = await _config.getParameter<String>('app.version') ?? '1.0.0';
    final clientPlatform = Platform.isAndroid ? 'android' :
                          Platform.isIOS ? 'ios' :
                          Platform.isWindows ? 'windows' : 'unknown';
  ...
  Future<HttpClient> _buildEnhancedHttpClient() async {
    ...
    // Connection settings
    final connectionTimeout = await _config.getParameter<int>('supabase.connection_timeout') ?? 30;
    client.connectionTimeout = Duration(seconds: connectionTimeout);

    // SSL verification
    final sslVerification = await _config.getParameter<bool>('supabase.ssl_verification') ?? true;
    if (!sslVerification) {
