...
class AdvancedCacheManager {
  ...
  Future<void> initialize() async {
    ...
    try {
      _logger.info(
          'Initializing Advanced Cache Manager', 'AdvancedCacheManager');
      ...
      _isInitialized = true;
      _logger.info('Advanced Cache Manager initialized successfully',
      ...
      _logger.error(
          'Failed to initialize cache manager', 'AdvancedCacheManager',
  ...
  Future<CacheEntry?> get(String key,
      {CacheStrategy strategy = CacheStrategy.auto}) async {
    ...
    try {
      ...
      _logger.warning('Cache get error for key $key: ${e.toString()}',
          'AdvancedCacheManager');
      return null;
    }
  }
  ...
  Future<void> set(
    ...
    await _setMultiLevel(key, data,
        strategy: strategy, ttl: ttl, compress: compress, encrypt: encrypt);
  }

  /// Set data across multiple cache levels
  Future<void> _setMultiLevel(
    String key,
    dynamic data, {
    CacheStrategy strategy = CacheStrategy.auto,
    Duration? ttl,
    ...
    try {
      ...
      if (strategy != CacheStrategy.diskOnly &&
        ...
        await _memoryCache.set(key, data,
            ttl: effectiveTtl, compress: compress, encrypt: encrypt);
      }

      // Disk cache
      if (strategy != CacheStrategy.memoryOnly &&
          strategy != CacheStrategy.networkOnly) {
        await _diskCache.set(key, data,
            ttl: effectiveTtl, compress: compress, encrypt: encrypt);
      }
      ...
      if (strategy == CacheStrategy.networkOnly ||
        ...
        await _networkCache.set(key, data,
            ttl: effectiveTtl, compress: compress, encrypt: encrypt);
      }
    } catch (e) {
      _logger.error('Cache set error for key $key: ${e.toString()}',
          'AdvancedCacheManager');
    }
  }
  ...
  Future<void> invalidate(String key) async {
    ...
    try {
      ...
      _logger.warning('Cache invalidation error for key $key: ${e.toString()}',
          'AdvancedCacheManager');
    }
  }
  ...
  Future<void> clearAll() async {
    ...
    try {
      ...
      _logger.error(
          'Cache clear error: ${e.toString()}', 'AdvancedCacheManager');
    }
  }
  ...
  CacheStatistics getStatistics() {
    ...
    return CacheStatistics(
      ...
      totalSize: _memoryCache.totalSize +
          _diskCache.totalSize +
          _networkCache.totalSize,
  ...
  Future<void> warmUpCache(List<String> keys) async {
    ...
    _logger.info('Starting cache warm-up for ${keys.length} keys',
        'AdvancedCacheManager');
  ...
  Future<void> prefetch(List<String> urls, {Duration? ttl}) async {
    ...
    _logger.info('Starting cache prefetch for ${urls.length} URLs',
        'AdvancedCacheManager');
  ...
  Future<String> exportCache() async {
    ...
    try {
      ...
      _logger.error(
          'Cache export error: ${e.toString()}', 'AdvancedCacheManager');
      return '{}';
    }
  }
  ...
  Future<void> importCache(String jsonData) async {
    ...
    try {
      ...
      _logger.error(
          'Cache import error: ${e.toString()}', 'AdvancedCacheManager');
    }
  }
  ...
  Future<void> _loadCacheConfiguration() async {
    ...
    final memoryCacheSize =
        _config.getParameter('cache.memory.max_entries', defaultValue: 100);
    final diskCacheSize =
        _config.getParameter('cache.disk.max_entries', defaultValue: 1000);
    final defaultTtlMinutes =
        _config.getParameter('cache.default_ttl_minutes', defaultValue: 30);
  ...
  void _startCacheMaintenance() {
    ...
    Timer.periodic(const Duration(minutes: 30), (_) async {
      try {
        ...
        _logger.warning(
            'Cache maintenance error: ${e.toString()}', 'AdvancedCacheManager');
      }
    });
  }
  ...
  static CacheEntry? fromJson(Map<String, dynamic> json) {
    try {
      return CacheEntry(
        ...
        ttl: DateTime.parse(json['expires'])
            .difference(DateTime.parse(json['created'])),
...
class MemoryCache {
  ...
  int get totalSize =>
      _cache.values.fold(0, (sum, entry) => sum + _estimateSize(entry.data));
...
class DiskCache {
  ...
  int get totalSize {
    ...
    try {
      return _cacheDir!
          .listSync()
          .fold(0, (sum, file) => sum + (file as File).lengthSync());
    } catch (e) {
      return 0;
    }
  }
...
class NetworkCache {
  ...
  int get size => _cache.length;
  int get totalSize => _cache.values
      .fold(0, (sum, entry) => sum + jsonEncode(entry.data).length);
...
final cacheGetProvider =
    FutureProvider.family<CacheEntry?, String>((ref, key) async {
  final cacheManager = ref.watch(cacheManagerProvider);
