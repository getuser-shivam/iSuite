...
class RealtimeCollaborationSystem {
  static final RealtimeCollaborationSystem _instance =
  ...
  Future<CollaborationSession> createSession({
    ...
    try {
      ...
      // Update presence
      await _presenceManager.updatePresence(
          creatorId, sessionId, PresenceStatus.active);
  ...
  Future<void> joinSession(String sessionId, String userId) async {
    try {
      ...
      // Update presence
      await _presenceManager.updatePresence(
          userId, sessionId, PresenceStatus.active);
  ...
  Future<void> leaveSession(String sessionId, String userId) async {
    try {
      ...
      final updatedParticipants =
          session.participants.where((id) => id != userId).toList();
  ...
  Future<void> updatePresence(
    ...
    try {
      await _presenceManager.updatePresence(userId, sessionId, status,
          data: data);
  ...
  Future<void> synchronizeOfflineChanges(String sessionId) async {
    try {
      final offlineOperations =
          await _offlineManager.getQueuedOperations(sessionId);
  ...
  Future<CollaborationSession?> _getSessionFromDatabase(
  ...
  Future<void> _saveOperationToDatabase(
      String sessionId, Operation operation) async {
  ...
  /// Helper methods
  Future<List<Operation>> _getConcurrentOperations(
      String sessionId, Operation operation) async {
    // Get operations that happened after the base version
    final operations = await _getOperationsFromDatabase(sessionId);
    return operations
        .where((op) => op.timestamp.isAfter(operation.timestamp))
        .toList();
  }

  Future<void> _applyOperationLocally(
      String sessionId, Operation operation) async {
  ...
  Future<void> _broadcastOperation(
      String sessionId, String userId, Operation operation) async {
...
class OperationalTransformer {
  Future<Operation> transform(
      Operation operation, List<Operation> concurrentOperations) async {
    ...
    for (final concurrentOp in concurrentOperations) {
      transformedOperation =
          _transformOperations(transformedOperation, concurrentOp);
    }
...
abstract class CollaborationEvent {
  ...
  factory CollaborationEvent.sessionEnded(String sessionId) = SessionEndedEvent;
...
class UserJoinedEvent extends CollaborationEvent {
  ...
  UserJoinedEvent(this.sessionId, this.userId)
      : super('user_joined', DateTime.now());
}
...
class UserLeftEvent extends CollaborationEvent {
  ...
  UserLeftEvent(this.sessionId, this.userId)
      : super('user_left', DateTime.now());
}
...
class MessageReceivedEvent extends CollaborationEvent {
  ...
  MessageReceivedEvent(this.message)
      : super('message_received', DateTime.now());
}
