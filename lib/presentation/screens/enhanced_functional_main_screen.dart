...
class FunctionalHomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: ParameterizedAppBar(
        title: configProvider._config.getParameter('app.name', defaultValue: 'iSuite'),
        actions: [
          IconButton(
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Welcome to ${configProvider._config.getParameter('app.name', defaultValue: 'iSuite')}',
            ...
            Text(
              configProvider._config.getParameter('app.tagline', defaultValue: 'Your comprehensive file management and network sharing solution'),
            ...
            Row(
              children: [
                ...
                const Spacer(),
                Text(
                  'Version ${configProvider._config.getParameter('app.version', defaultValue: '2.0.0')}',
                  style: TextStyle(
                    color: Colors.grey[600],
                    fontSize: 12,
                  ),
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                _buildStatusItem(
                  'File System',
                  _getFileSystemStatus(configProvider),
                  Icons.storage,
                ...
                _buildStatusItem(
                  'Network Services',
                  _getNetworkStatus(configProvider),
                  Icons.wifi,
  ...
  String _getFormattedFileCount(configProvider) {
    final count = configProvider._config.getParameter('files.total_count', defaultValue: 1234);
    return count.toString();
  }

  String _getDeviceCount(configProvider) {
    final count = configProvider._config.getParameter('network.device_count', defaultValue: 8);
    return count.toString();
  }

  String _getActiveTransfers(configProvider) {
    final count = configProvider._config.getParameter('transfers.active_count', defaultValue: 3);
    return count.toString();
  }

  String _getStorageUsage(configProvider) {
    final used = configProvider._config.getParameter('storage.used_mb', defaultValue: 2300);
    return '${(used / 1024).toStringAsFixed(1)} GB';
  }

  String _getFileSystemStatus(configProvider) {
    return configProvider._config.getParameter('system.filesystem_status', defaultValue: 'Operational');
  }

  String _getNetworkStatus(configProvider) {
    return configProvider._config.getParameter('system.network_status', defaultValue: 'Active');
  }

  String _getAIStatus(configProvider) {
    return configProvider._config.getParameter('system.ai_status', defaultValue: 'Ready');
  }

  String _getStorageStatus(configProvider) {
    final used = configProvider._config.getParameter('storage.used_mb', defaultValue: 2300);
    final total = configProvider._config.getParameter('storage.total_mb', defaultValue: 5120);
    final percentage = ((used / total) * 100).toInt();
    return '$percentage% used';
  }

  Color _getFileSystemStatusColor(configProvider) {
    final status = configProvider._config.getParameter('system.filesystem_status', defaultValue: 'Operational');
  ...
  Color _getNetworkStatusColor(configProvider) {
    final status = configProvider._config.getParameter('system.network_status', defaultValue: 'Active');
    switch (status.toLowerCase()) {
      case 'active':
        return Colors.green;
      case 'connecting':
  ...
  Color _getAIStatusColor(configProvider) {
    final status = configProvider._config.getParameter('system.ai_status', defaultValue: 'Ready');
    switch (status.toLowerCase()) {
      case 'ready':
        return Colors.blue;
      case 'processing':
