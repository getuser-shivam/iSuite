...
class _AIAnalysisPageState extends State<AIAnalysisPage> with TickerProviderStateMixin {
  ...
  void _initializeAnalysisData() {
    ...
    _fileCategories = [
      {'name': 'Documents', 'count': 234, 'size': '2.4 GB', 'color': Colors.blue},
      {'name': 'Images', 'count': 456, 'size': '8.7 GB', 'color': Colors.green},
      {'name': 'Videos', 'count': 89, 'size': '24.3 GB', 'color': Colors.orange},
      {'name': 'Music', 'count': 167, 'size': '12.1 GB', 'color': Colors.purple},
      {'name': 'Applications', 'count': 23, 'size': '3.2 GB', 'color': Colors.red},
      {'name': 'Archives', 'count': 78, 'size': '5.8 GB', 'color': Colors.teal},
    ];

    _organizationSuggestions = [
      {'type': 'consolidate', 'title': 'Consolidate Document Folders', 'description': 'Merge 5 document folders into organized structure', 'impact': 'High', 'files': 89},
      {'type': 'archive', 'title': 'Archive Old Photos', 'description': 'Move photos older than 2 years to archive', 'impact': 'Medium', 'files': 234},
      {'type': 'delete', 'title': 'Remove Temporary Files', 'description': 'Delete 156 unused temporary files', 'impact': 'Low', 'files': 156},
      {'type': 'rename', 'title': 'Standardize Naming', 'description': 'Apply consistent naming to 67 files', 'impact': 'Medium', 'files': 67},
    ];
  }
                  ...
                  children: [
                    ...
                    if (_isAnalyzing) ...[
                      ...
                      Row(
                        ...
                        children: [
                          _buildMetricItem('Files', _analysisResults['totalFiles'].toString()),
                          _buildMetricItem('Categories', _analysisResults['categories'].toString()),
                          _buildMetricItem('Score', '${_analysisResults['organizationScore']}%'),
                        ],
                      ),
                    ],
                  ],
                ...
                return SlideInAnimation(
                  ...
                  child: Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text('${category['count']} files • ${category['size']}'),
                      trailing: Row(
                          ...
                          Row(
                            children: [
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: _getSuggestionColor(suggestion['impact']).withOpacity(0.2),
                                  borderRadius: BorderRadius.circular(8),
                                ),
                                child: Icon(
                                  _getSuggestionIcon(suggestion['type']),
                                  color: _getSuggestionColor(suggestion['impact']),
                                ),
                              ),
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: _getSuggestionColor(suggestion['impact']).withOpacity(0.1),
                                  borderRadius: BorderRadius.circular(12),
                                  border: Border.all(
                                    color: _getSuggestionColor(suggestion['impact']).withOpacity(0.3),
                                  ),
                                ),
                                child: Text(
                                  suggestion['impact'],
              ...
              child: Column(
                children: [
                  _buildInsightSection(
                    'File Usage Patterns',
                    [
                      _buildPatternItem('Most active folder', 'Documents/Projects', '89% of recent activity'),
                      _buildPatternItem('Peak usage time', '2-4 PM weekdays', 'Highest file operations'),
                      _buildPatternItem('Common file types', 'PDF, DOCX, Images', '75% of total files'),
                      _buildPatternItem('Storage growth', '+2.3 GB/month', 'Average monthly increase'),
                    ],
                  ),
                  const SizedBox(height: 24),
                  _buildInsightSection(
                    ...
                    [
                      _buildPatternItem('Space savings', '3.2 GB available', 'Delete unused files'),
                      _buildPatternItem('Duplicate files', '23 sets found', 'Consolidate duplicates'),
                      _buildPatternItem('Large files', '45 files >100MB', 'Consider compression'),
                      _buildPatternItem('Archive candidates', '156 files unused', 'Move to archive'),
                    ],
                  ),
                  const SizedBox(height: 24),
                  _buildInsightSection(
                    'Security Insights',
                    [
                      _buildPatternItem('Shared files', '67 files accessible', 'Review permissions'),
                      _buildPatternItem('Backup coverage', '78% backed up', 'Increase backup coverage'),
                      _buildPatternItem('Sensitive data', '12 files flagged', 'Check for PII'),
                      _buildPatternItem('Access patterns', 'Normal activity', 'No unusual access detected'),
                    ],
                  ),
                ],
              ),
  ...
  Widget _buildRecentActivity() {
    final activities = [
      {'action': 'Analyzed 892 files', 'time': '2m ago', 'icon': Icons.search, 'color': Colors.blue},
      {'action': 'Found 23 duplicates', 'time': '5m ago', 'icon': Icons.copy, 'color': Colors.orange},
      {'action': 'Generated organization suggestions', 'time': '8m ago', 'icon': Icons.lightbulb, 'color': Colors.green},
      {'action': 'Completed usage pattern analysis', 'time': '12m ago', 'icon': Icons.analytics, 'color': Colors.purple},
    ];
  ...
  void _startAIAnalysis() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('AI analysis completed! Check insights for recommendations.'),
