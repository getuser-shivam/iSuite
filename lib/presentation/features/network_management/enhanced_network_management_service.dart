...
class NetworkManagementService {
  ...
  Future<NetworkResponse> makeRequest(
    ...
    final effectiveTimeout = timeout ?? await _getRequestTimeout();
    ...
    try {
      ...
      request.followRedirects = await _config.getParameter<bool>('network.http.follow_redirects_auto') ?? true;
      request.maxRedirects = await _config.getParameter<int>('network.http.max_redirects') ?? 5;
      ...
      request.headers.set(
          'User-Agent',
          await _config.getParameter<String>('network.http.user_agent_custom') ?? 'iSuite/2.0.0');

      // Handle compression
      if (compress &&
          (await _config.getParameter<bool>('network.http.send_compression') ?? true)) {
  ...
  Future<WebSocketConnection> connectWebSocket(
    ...
    final effectiveAutoReconnect = autoReconnect &&
        (await _config.getParameter<bool>('network.websocket.auto_reconnect') ?? true);
    final effectivePingInterval = pingInterval ??
        Duration(
            milliseconds: (await _config.getParameter<int>('network.websocket.heartbeat_interval') ?? 30000));
    ...
    try {
      ...
      if ((await _config.getParameter<int>('network.websocket.heartbeat_interval') ?? 30000) > 0) {
  ...
  NetworkConfiguration getConfiguration() {
    return NetworkConfiguration(
      ...
      keepAliveDuration: Duration(seconds: 300), // Default value
      idleTimeout: Duration(seconds: 60), // Default value
      maxConcurrentRequests: 10, // Default value
      requestTimeout: Duration(seconds: 30), // Default synchronous timeout
      retryBackoffMultiplier: 2.0, // Default value
      circuitBreakerThreshold: 5, // Default value
      enableCompression: true, // Default value
      sslVerificationStrict: true, // Default value
      cacheEnabled: true, // Default value
      webSocketAutoReconnect: true, // Default value
    );
  }
  ...
  Future<void> _initializeHttpClients() async {
    ...
    for (final config in poolConfigs) {
      ...
      client.connectionTimeout = await _getRequestTimeout();
      client.idleTimeout = Duration(
          seconds: (await _config.getParameter<int>('network.connection.idle_timeout')) ?? 60);
      final maxConcurrent = await _config.getParameter<int>('network.connection.max_concurrent_requests') ?? 10;
      client.maxConnectionsPerHost = maxConcurrent;

      // SSL configuration
      final sslStrict = await _config.getParameter<bool>('network.http.ssl_verification_strict') ?? true;
      if (sslStrict) {
  ...
  Future<Duration> _getRequestTimeout() async {
    final baseTimeout = (await _config.getParameter<int>('network.connection.request_timeout_base')) ?? 30;
    final maxTimeout = (await _config.getParameter<int>('network.connection.request_timeout_max')) ?? 300;
...
class WebSocketConnection {
  ...
  void startHeartbeat(Duration interval) {
    _heartbeatTimer = Timer.periodic(interval, (_) {
      try {
        ...
        // Just check if connection is still alive by attempting a simple operation
        if (webSocket.readyState != WebSocket.open) {
          throw Exception('WebSocket connection is not open');
        }
      } catch (e) {
        // Heartbeat failed, connection might be dead
        _logger.warning('WebSocket heartbeat failed: $e', 'NetworkManagementService');
  ...
  void reconnect() {
    ...
    _logger.info('Attempting WebSocket reconnection to $url', 'NetworkManagementService');
    // This would trigger the parent service to reconnect
  }
