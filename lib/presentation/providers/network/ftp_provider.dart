...
import '../../../../../core/logging/logging_service.dart'; // Import logging service for robustness
...
class FtpStateNotifier extends StateNotifier<FtpState> {
  ...
  void _updateState(FtpState newState) {
    state = newState;
  }
  ...
  Future<void> connect(FtpConnection connection) async {
    ...
    try {
      ...
      _logger.info('FTP connected successfully to ${connection.host}:${connection.port}');
      ...
      _logger.error('FTP connection failed for ${connection.host}:${connection.port}: $e');
      state = state.copyWith(
        isConnected: false,
        isLoading: false,
  ...
  Future<void> listFiles(String path) async {
    ...
    try {
      ...
      _logger.info('FTP listed ${files.length} files in $path');
      state = state.copyWith(
        files: files,
        currentPath: path,
        isLoading: false,
      ...
      _logger.error('FTP listFiles failed for $path: $e');
      state = state.copyWith(
        isLoading: false,
...
final ftpStateProvider = StateNotifierProvider<FtpStateNotifier, FtpState>((ref) {
  // In a real implementation, you'd inject the repository through dependency injection
  throw UnimplementedError('FTP repository needs to be injected');
});
