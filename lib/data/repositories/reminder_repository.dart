...
class ReminderRepository {
  static Future<List<ReminderModel>> getAllReminders() async {
    ...
    final List<Map<String, dynamic>> maps = await db.query('reminders', orderBy: 'due_date ASC');
  ...
  static Future<List<ReminderModel>> getUpcomingReminders({int hours = 24}) async {
    final now = DateTime.now().millisecondsSinceEpoch;
  ...
  static Future<Map<String, int>> getReminderStats() async {
    ...
    final activeResult = await db.rawQuery('SELECT COUNT(*) FROM reminders WHERE status = ?', [ReminderStatus.active.name]);
    final active = Sqflite.firstIntValue(activeResult) ?? 0;

    final completedResult = await db.rawQuery('SELECT COUNT(*) FROM reminders WHERE status = ?', [ReminderStatus.completed.name]);
    final completed = Sqflite.firstIntValue(completedResult) ?? 0;

    final overdueResult = await db.rawQuery('SELECT COUNT(*) FROM reminders WHERE status = ? AND due_date < ?', [ReminderStatus.active.name, DateTime.now().millisecondsSinceEpoch]);
  ...
  static ReminderModel _fromMap(Map<String, dynamic> map) {
    return ReminderModel(
      id: map['id'],
      title: map['title'],
      description: map['description'],
      dueDate: DateTime.fromMillisecondsSinceEpoch(map['due_date']),
      repeat: ReminderRepeat.values.firstWhere((r) => r.name == map['repeat']),
      priority: ReminderPriority.values.firstWhere((p) => p.name == map['priority']),
      status: ReminderStatus.values.firstWhere((s) => s.name == map['status']),
      createdAt: DateTime.fromMillisecondsSinceEpoch(map['created_at']),
      updatedAt: DateTime.fromMillisecondsSinceEpoch(map['updated_at']),
      snoozeUntil: map['snooze_until'] != null ? DateTime.fromMillisecondsSinceEpoch(map['snooze_until']) : null,
