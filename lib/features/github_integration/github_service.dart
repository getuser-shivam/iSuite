...
class GitHubService {
  ...
  Future<List<RepositoryCommit>> getCommitHistory(String owner, String repo,
  ...
  Future<String> _generateAIAnalysis(List<RepositoryCommit> commits,
      Map<String, int> authors, List<DateTime> dates) async {
    // Prepare commit messages for AI analysis
    final commitMessages =
        commits.take(10).map((c) => c.commit?.message ?? '').join('\n');
...
''';
    ...
    final analysisPrompt = prompt
        ...
        .replaceAll('{range}',
            dates.isNotEmpty ? '${dates.first} to ${dates.last}' : 'Unknown');

    return await _aiService.generateContent(analysisPrompt);
  }

  Future<void> updateReadme(
      String owner, String repo, String newContent) async {
    if (_github == null) throw Exception('Not authenticated');
  ...
  Future<void> commitAndPush(String owner, String repo, String message) async {
    ...
    await updateReadme(
        owner, repo, 'Committed via iSuite AI-enhanced workflow\n$message');
  }
...
''';
    ...
    return lines
        .where((line) =>
            line.trim().isNotEmpty && RegExp(r'^\d+\.').hasMatch(line))
