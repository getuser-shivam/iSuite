...
class _FileManagementScreenState extends State<FileManagementScreen> 
  ...
  Future<void> _initializeFromConfig() async {
    ...
    setState(() {
      _configParameters = {
        'max_file_size': CentralConfig.instance.getParameter('max_file_size', 100 * 1024 * 1024),
        'allowed_file_types': CentralConfig.instance.getParameter('allowed_file_types', ['pdf', 'doc', 'docx', 'txt', 'jpg', 'png']),
        'enable_encryption': CentralConfig.instance.getParameter('enable_encryption', false),
        'theme_mode': CentralConfig.instance.getParameter('theme_mode', 'system'),
        'primary_color': CentralConfig.instance.getParameter('primary_color', '#1976D2'),
      };
    });
  }
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'iSuite File Manager',
          style: TextStyle(
            color: _configParameters['theme_mode'] == 'dark' ? Colors.white : Colors.black,
          ),
        ),
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  _buildConfigRow('Max File Size', '${(_configParameters['max_file_size'] as int) / (1024 * 1024)} MB'),
                  _buildConfigRow('Allowed Types', '${(_configParameters['allowed_file_types'] as List<String>).length} types'),
                  _buildConfigRow('Encryption', _configParameters['enable_encryption'] ? 'Enabled' : 'Disabled'),
                  _buildConfigRow('Theme', _configParameters['theme_mode']),
                  _buildConfigRow('Primary Color', _configParameters['primary_color']),
                ],
              ),
            ),
            const SizedBox(height: 20),
