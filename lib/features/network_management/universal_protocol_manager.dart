...
class TransferProgress {
  ...
  double get speed =>
      duration.inSeconds > 0 ? transferredBytes / duration.inSeconds : 0.0;
...
class UniversalProtocolManager {
  ...
  final StreamController<ConnectionInfo> _connectionStatusController =
      StreamController.broadcast();
  ...
  Future<void> initialize() async {
    ...
    try {
      _logger.info(
          'Initializing Universal Protocol Manager', 'ProtocolManager');

      // Register with CentralConfig
      await _config.registerComponent('UniversalProtocolManager', '1.0.0',
      ...
      _logger.error(
          'Failed to initialize Universal Protocol Manager', 'ProtocolManager',
  ...
  Future<void> _initializeProtocolHandlers() async {
    ...
    _logger.info(
        'Protocol handlers initialized for ${NetworkProtocol.values.length} protocols',
        'ProtocolManager');
  }
  ...
  Future<ConnectionInfo> connect(ConnectionConfig config) async {
    ...
    try {
      _logger.info(
          'Connecting to ${config.protocol.name}://${config.host}:${config.port}',
          'ProtocolManager');
      ...
      if (handler == null) {
        throw ProtocolException(
            'Protocol ${config.protocol.name} not supported');
      }
      ...
      if (result) {
        ...
        _logger.info(
            'Successfully connected to ${config.protocol.name}://${config.host}:${config.port}',
            'ProtocolManager');
      ...
      await _security.logAuditEntry(
          'connection_failed', 'Network connection failed', additionalData: {
        'protocol': config.protocol.name,
        'host': config.host,
        'error': e.toString()
      });
      ...
      _logger.error(
          'Failed to connect to ${config.protocol.name}://${config.host}:${config.port}: $e',
          'ProtocolManager');
  ...
  Future<void> disconnect(String connectionId) async {
    ...
    try {
      ...
      await _security.logAuditEntry(
          'connection_closed', 'Network connection closed',
          additionalData: {'connection_id': connectionId});
    } catch (e) {
      _logger.error('Error disconnecting connection $connectionId: $e',
          'ProtocolManager');
    }
  }
  ...
  Future<List<FileSystemEntity>> listDirectory(
      String connectionId, String path) async {
    final connectionInfo = _activeConnections[connectionId];
    ...
    return await _circuitBreaker.execute(
      ...
      operation: () async {
        ...
        await _security.logAuditEntry(
            'directory_listed', 'Directory contents listed', additionalData: {
  ...
  Future<void> downloadFile(
      String connectionId, String remotePath, String localPath) async {
    final connectionInfo = _activeConnections[connectionId];
    if (connectionInfo == null) {
      throw ProtocolException('Connection not found: $connectionId');
    }
    ...
    try {
      await _circuitBreaker.execute(
        ...
        operation: () async {
          ...
          await _security.logAuditEntry(
              'file_downloaded', 'File downloaded from remote server',
              additionalData: {
                'connection_id': connectionId,
  ...
  Future<void> uploadFile(
      String connectionId, String localPath, String remotePath) async {
    final connectionInfo = _activeConnections[connectionId];
    if (connectionInfo == null) {
      throw ProtocolException('Connection not found: $connectionId');
    }
    ...
    try {
      await _circuitBreaker.execute(
        ...
        operation: () async {
          ...
          await _security.logAuditEntry(
              'file_uploaded', 'File uploaded to remote server',
              additionalData: {
                'connection_id': connectionId,
                'local_path': localPath,
                'remote_path': remotePath,
  ...
  Future<void> createDirectory(String connectionId, String remotePath) async {
    ...
    await _circuitBreaker.execute(
      ...
      operation: () async {
        ...
        await _security.logAuditEntry(
            'directory_created', 'Directory created on remote server',
            additionalData: {
              'connection_id': connectionId,
              'path': remotePath
            });
  ...
  Future<void> delete(String connectionId, String remotePath) async {
    ...
    await _circuitBreaker.execute(
      ...
      operation: () async {
        ...
        await _security.logAuditEntry(
            'file_deleted', 'File/directory deleted from remote server',
            additionalData: {
              'connection_id': connectionId,
              'path': remotePath
            });
  ...
  Map<String, dynamic> getConnectionStatistics() {
    final stats = {
      ...
      'connection_pool_size':
          _connectionPools.values.expand((pool) => pool).length,
    };

    for (final connection in _activeConnections.values) {
      final protocol = connection.config.protocol.name;
      stats['connections_by_protocol'][protocol] =
          (stats['connections_by_protocol'][protocol] ?? 0) + 1;
  ...
  Map<String, dynamic> getTransferStatistics() {
    final completedTransfers =
        _activeTransfers.values.where((t) => t.endTime != null).toList();
    final avgSpeed = completedTransfers.isEmpty
        ? 0.0
        : completedTransfers.map((t) => t.speed).reduce((a, b) => a + b) /
    ...
    return {
      ...
      'total_bytes_transferred':
          _activeTransfers.values.fold(0, (sum, t) => sum + t.transferredBytes),
    };
  }
  ...
  String _generateConnectionId(ConnectionConfig config) {
    final data =
        '${config.protocol.name}_${config.host}_${config.port}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Future<void> _addToConnectionPool(ConnectionInfo connection) async {
    if (!await _config.getParameter('protocols.connection_pool.enabled',
        defaultValue: true)) {
      return;
    }

    final poolKey =
        '${connection.config.protocol.name}_${connection.config.host}_${connection.config.port}';
    _connectionPools.putIfAbsent(poolKey, () => []).add(connection);

    // Set idle timeout
    final idleTimeout = Duration(
        seconds: await _config.getParameter(
            'protocols.connection_pool.idle_timeout',
            defaultValue: 300));
...
abstract class ProtocolHandler {
  ...
  Future<void> downloadFile(
      String connectionId, String remotePath, String localPath,
      {void Function(int transferred, int total)? onProgress});
  Future<void> uploadFile(
      String connectionId, String localPath, String remotePath,
      {void Function(int transferred, int total)? onProgress});
